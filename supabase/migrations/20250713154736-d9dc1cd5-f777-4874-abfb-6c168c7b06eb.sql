-- Fix unrealistic daily session targets and add validation
-- Update today's entries that have unrealistic targets (>10) to default of 3
UPDATE daily_progress 
SET daily_session_target = 3 
WHERE date = CURRENT_DATE 
  AND daily_session_target > 10;

-- Create a function to validate session targets
...
-- Update the existing function to use validation
...
BEGIN
  ...
  session_target := validate_session_target(existing_target);
  ...
  ON CONFLICT (user_id, date) DO UPDATE SET
    ...
    daily_session_target = validate_session_target(COALESCE(daily_progress.daily_session_target, EXCLUDED.daily_session_target)),
    updated_at = now();
END;
$$;