...
-- Funktion zum Aktualisieren der Belohnungen
...
BEGIN
  ...
  -- Update oder Insert Belohnungsdaten
  ...
  VALUES (
    ...
    today_date,
    CASE 
      WHEN completed_steps_count >= 1 AND completed_steps_count < 5 THEN '["Trainings-Starter"]'::jsonb
      WHEN completed_steps_count >= 5 AND completed_steps_count < 10 THEN '["Trainings-Starter", "Fleißiger Trainer"]'::jsonb
      WHEN completed_steps_count >= 10 THEN '["Trainings-Starter", "Fleißiger Trainer", "Trainings-Experte"]'::jsonb
      ELSE '[]'::jsonb
    END
  )
...
-- Trigger-Funktion für automatische Belohnungs-Updates
CREATE OR REPLACE FUNCTION trigger_update_rewards()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  -- Finde user_id über training_plan
  PERFORM update_user_rewards(
...
-- Trigger erstellen für automatische Updates beim Abschließen von Schritten
DROP TRIGGER IF EXISTS on_training_step_completed ON training_steps;
CREATE TRIGGER on_training_step_completed
  AFTER UPDATE OF is_completed ON training_steps
  FOR EACH ROW
  WHEN (NEW.is_completed = true AND OLD.is_completed = false)
  EXECUTE FUNCTION trigger_update_rewards();
